...
### Python
Program was developed for [Python 3.6](https://www.python.org/). As of June 2018 program was not tested with other versions of Python.
...
- `File` menu's options:
    - `Load Sheet` load data from [CSV spreadsheet](#spreadsheets)
    - `Load Original` load data from directory ([original-like files]((#game-readable-files))
    ...
    - [`Save to Original / Changed`](#exclusive-saving) only save provinces that were modified
...
### Side bar
Side bar contains various buttons. It is here for future use. The buttons are placeholders.
...
# Configurator
To open the Configurator, click `Main` menu and choose `Run Configurator` option.
...
### Display
Program displays data representation in the central frame. In configurator there are a few options that affect the way program does it. These are:
...
- `Hide provinces with specified super-regions` and `Hidden super-regions`: Olny works if the Checkbox is checked. If province is assigned to super-region in the list, it will be hidden.
- `Hidden columns`: List of columns to hide.



# Commands
Program can receive commands from Command Line and Command Files. This section explains how to use commands.
...
#### Comments
To add a comment in Commands File use `>#`. Comments end when next `>` character is found. Example:
```
> command
># This is comment
...
### Update load
Updates currently loaded data with content from sheet / directory.
...
# Selections
All modifying actions are executed on selection, instead of main data. To create selection use [Selection menu](#top-bar) or [Selection commands](#data-manipulation-commands). There are three selection related actions user can take.
- Create new selection: Creates selection with all provinces that fulfill requirements.
- Create sub-selection: Creates selection with provinces from current selection, that fulfill requirements.
- Append to selection: Appends all provinces that fulfill requirements to current selection.
